...
fn get_tuple(data: &str) -> (String, usize, Option<Vec<String>>) {
    let joined_data = data.split_whitespace().collect::<Vec<&str>>();

    let name = joined_data[0];

    let num = joined_data[1]
        .trim_start_matches('(')
        .trim_end_matches(')')
        .parse::<usize>()
        .unwrap();

    if joined_data.len() > 3 {
        (
            name.to_owned(),
            num,
            Some(
                joined_data[3..]
                    .iter()
                    .map(|&leafs| leafs.trim_end_matches(',').to_owned())
                    .collect::<Vec<String>>(),
            ),
        )
    } else {
        (name.to_owned(), num, None)
    }
}
...
cntj (57)";
    ...
    assert_eq!("tknk".to_owned(), result);
