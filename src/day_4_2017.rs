...
fn get_char_frequencies(word: &str) -> HashSet<(char, usize)> {
    let mut counts: HashSet<(char, usize)> = HashSet::with_capacity(word.len() + 2);

    for c in word.chars() {
        let char_count = count_char_on_word(c, word);
        counts.insert(char_count);
    }
