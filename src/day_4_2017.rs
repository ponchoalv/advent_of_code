...
fn is_valid_passphrase(passphrase: &str) -> bool {
    ...
    words.len() == passphrase.split_whitespace().count()
}
...
fn is_valid_passphrase_part_2(passphrase: &str) -> bool {
    ...
    let is_diferent_with_anagrams = (1..words.len()).map(|x| {
        words.iter().zip(words.iter().cycle().skip(x)).filter(|(a, b)| {
            if a.len() == b.len() {
                let a = a.chars().

                let common_chars = a.intersection(&b).count();
                common_chars == a.len() && common_chars == b.len()
            } else { false }
        }).count() > 0
    }).filter(|&is_dirferent| {
        is_dirferent
    ...
    is_valid_passphrase(passphrase) && is_diferent_with_anagrams
}
...
fn probar_day_4_2_2017() {
    ...
    let result = is_valid_passphrase_part_2(input);
    assert_eq!(true, result);
